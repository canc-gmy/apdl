...
class Analysis:
    def __init__(self, source, tol=1e-7):
        if isinstance(source, Plate):
            ...
            raise ValueError("source must be a Plate object or a valid Parquet file path")
    ...
    def find_sampling_freq_exx(self, threshold=0.99):
        psd_exx = np.abs(self.ft_exx)**2

        energy_profile_x = np.sum(psd_exx, axis=0)
        cumulative_energy_x = np.cumsum(energy_profile_x) / np.sum(energy_profile_x)
        idx_cutoff_x = np.argmax(cumulative_energy_x >= threshold)
        fs_max_x = np.abs(self.kx[idx_cutoff_x])

        energy_profile_y = np.sum(psd_exx, axis=1)
        cumulative_energy_y = np.cumsum(energy_profile_y) / np.sum(energy_profile_y)
        idx_cutoff_y = np.argmax(cumulative_energy_y >= threshold)
        fs_max_y = np.abs(self.ky[idx_cutoff_y])

        print('================================================================================')
        print('RISULTATI ANALISI SPETTRALE e_xx ')
        print('--------------------------------------------------------------------------------')
        print('Direzione X')
        print(f"Numero d'onda critico (99% energia): {fs_max_x:.2f} 1/m")
        print(f"Grandezza mesh massima: {1 / (2 * fs_max_x) * 1000:.2f} mm")
        print('Direzione Y')
        print(f"Numero d'onda critico (99% energia): {fs_max_y:.2f} 1/m")
        print(f"Grandezza mesh massima: {1 / (2 * fs_max_y) * 1000:.2f} mm")

        return fs_max_x, fs_max_y

    def find_sampling_freq_exy(self, threshold=0.99):
        psd_exy = np.abs(self.ft_exy)**2

        energy_profile_x = np.sum(psd_exy, axis=0)
        cumulative_energy_x = np.cumsum(energy_profile_x) / np.sum(energy_profile_x)
        idx_cutoff_x = np.argmax(cumulative_energy_x >= threshold)
        fs_max_x = np.abs(self.kx[idx_cutoff_x])

        energy_profile_y = np.sum(psd_exy, axis=1)
        cumulative_energy_y = np.cumsum(energy_profile_y) / np.sum(energy_profile_y)
        idx_cutoff_y = np.argmax(cumulative_energy_y >= threshold)
        fs_max_y = np.abs(self.ky[idx_cutoff_y])

        print('================================================================================')
        print('RISULTATI ANALISI SPETTRALE e_xy ')
        print('--------------------------------------------------------------------------------')
        print('Direzione X')
        print(f"Numero d'onda critico (99% energia): {fs_max_x:.2f} 1/m")
        print(f"Grandezza mesh massima: {1 / (2 * fs_max_x) * 1000:.2f} mm")
